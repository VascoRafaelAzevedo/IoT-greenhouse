...
impl Database {
    ...
    pub async fn connect(config: &Config) -> Result<Self> {
        ...
        info!("Connecting to database: {}/{}", config.db_host, config.db_name);
    ...
    pub async fn insert_telemetry(&self, msg: &TelemetryMessage) -> Result<()> {
        ...
        let exists: bool = self.client
            .query_one(check_query, &[&msg.greenhouse_id, &msg.timestamp, &msg.sequence])
            .await
            .context("Failed to check for duplicate telemetry")?
            .get(0);
        
        if exists {
            info!("Duplicate telemetry ignored: greenhouse={}, time={}, seq={}", 
        ...
        if result == 1 {
            info!("Telemetry stored: greenhouse={}, seq={}", msg.greenhouse_id, msg.sequence);
        } else {
    ...
    pub async fn update_greenhouse_last_seen(&self, greenhouse_id: &Uuid, timestamp: &DateTime<Utc>) -> Result<()> {
